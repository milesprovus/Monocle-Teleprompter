import { receiveRawData } from "./repl.mjs";
import { replHandleResponse,onDisconnect } from "./repl.mjs";
import { nordicDfuHandleControlResponse } from './nordicdfu.mjs';
import util from 'util';
let device = null;
import pkg from './ble/index.js';
const { webbluetooth } = pkg;
var bluetooth = webbluetooth;
...
async function transmitReplData() {
    ...
    await replRxCharacteristic.writeValueWithoutResponse(new Uint8Array(payload))
        ...
        .catch((error) => {
            console.log("transmitReplData error: ", error);
